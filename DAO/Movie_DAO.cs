...
namespace DAO
{
    public class Movie_DAO
    {
        ...
        public static List<Movie_DTO> GetListMovieByDate(DateTime date)
        {
            List<Movie_DTO> listMovie = new List<Movie_DTO>();
            DataTable data = DataProvider.ExecuteQuery("select * from Phim where @Date <= NgayKetThuc", new object[] { date });
            foreach (DataRow row in data.Rows)
            {
                Movie_DTO movie = new Movie_DTO(row);
                listMovie.Add(movie);
            }
            return listMovie;
        }

        public static List<Movie_DTO> GetListMovie()
        {
            List<Movie_DTO> listMovie = new List<Movie_DTO>();
